...
for i, label in enumerate(labels):
    ...
    tk.Label(frame, text=f"{label} ({hint})", anchor='w', width=40).grid(row=i, column=0, sticky='w', padx=5, pady=2)
    entry = tk.Entry(frame, width=20)
    entry.grid(row=i, column=1, padx=5, pady=2)
    entries.append(entry)

predict_btn = tk.Button(root, text="Predict", command=predict, font=("Arial", 14), bg="#4CAF50", fg="white", width=20)
